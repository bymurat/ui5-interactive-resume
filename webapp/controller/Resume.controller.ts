...
export default class Resume extends BaseController {
	...
	public onContactChannelPress(oEvent: ListItemBase$PressEvent): void {
		const oListItem = oEvent.getSource();
		const oContext = oListItem.getBindingContext("resume");
		if (!oContext) {
			return;
		}
		const channel = oContext.getObject() as ContactChannel;
		if (channel?.href) {
	...
	private getPrimaryEmail(): ContactChannel | undefined {
		const resumeModel = this.getOwnerComponent().getModel(
			"resume",
		) as JSONModel;
		const channels = resumeModel.getProperty("/contact/channels") as
			| ContactChannel[]
			| undefined;
		if (!channels) {
			return undefined;
