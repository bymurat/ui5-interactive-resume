...
4. **Update the translated content too**: [webapp/model/resume.fr.json](../../../webapp/model/resume.fr.json) and [webapp/model/resume.de.json](../../../webapp/model/resume.de.json) are full French/German translations of the same shape (loaded instead of `resume.json` when the resolved UI5 language is `fr`/`de` — see `resolveContentLocale()` in [webapp/model/models.ts](../../../webapp/model/models.ts)). Keep `id`s, dates, `hat`, and `technologies` arrays identical across all three files — only translate human-facing prose (`summary`, `achievements`, `department`, `role`, `team`, labels). Luxembourgish (`lb`) only has translated UI chrome ([i18n_lb.properties](../../../webapp/i18n/i18n_lb.properties)), not translated CV content.
5. **Verify**: run `npm run ts-typecheck` — this validates any code binding to `resume.json` still compiles against the updated types. There is no JSON Schema validation, so the type-check is the only automated guardrail for shape correctness.
6. If the change affects `profile` or top-level SEO-relevant fields, note that [scripts/inject-seo.mjs](../../../scripts/inject-seo.mjs) reads `resume.json` at build time to generate the schema.org/Person JSON-LD — re-run `npm run build` (or `build:opt`) if verifying SEO output.
